...
namespace klibpp {
template <typename TFile, typename TFunc, typename TSpec>
class KStream;
...
class KStream<TFile, TFunc, mode::Out_> : public KStreamBase_ {
   ...
   public:
    KStream(TFile f_, TFunc func_, spec_type = mode::out, format::Format fmt_ = DEFAULT_FORMAT,
            std::make_unsigned_t<size_type> bs_ = DEFAULT_BUFSIZE, close_type cfunc_ = nullptr)
        : m_buf(new char_type[bs_]),
          w_buf(new char_type[bs_]),
          bufsize(bs_),
          bufslock(new std::mutex),
          cv(new std::condition_variable),
          wraplen(DEFAULT_WRAPLEN),
          fmt(fmt_),
          f(std::move(f_)),
          func(std::move(func_)),
          close(cfunc_) {
    ...
    KStream(TFile f_, TFunc func_, format::Format fmt_, std::make_unsigned_t<size_type> bs_ = DEFAULT_BUFSIZE,
            close_type cfunc_ = nullptr)
        : KStream(std::move(f_), std::move(func_), mode::out, fmt_, bs_, cfunc_) {}

    KStream(TFile f_, TFunc func_, spec_type, format::Format fmt_, close_type cfunc_)
        : KStream(std::move(f_), std::move(func_), mode::out, fmt_, DEFAULT_BUFSIZE, cfunc_) {}

    KStream(TFile f_, TFunc func_, format::Format fmt_, close_type cfunc_)
        : KStream(std::move(f_), std::move(func_), mode::out, fmt_, DEFAULT_BUFSIZE, cfunc_) {}

    KStream(TFile f_, TFunc func_, spec_type, std::make_unsigned_t<size_type> bs_, close_type cfunc_ = nullptr)
        : KStream(std::move(f_), std::move(func_), mode::out, DEFAULT_FORMAT, bs_, cfunc_) {}

    KStream(TFile f_, TFunc func_, std::make_unsigned_t<size_type> bs_, close_type cfunc_ = nullptr)
        : KStream(std::move(f_), std::move(func_), mode::out, DEFAULT_FORMAT, bs_, cfunc_) {}

    KStream(TFile f_, TFunc func_, spec_type, close_type cfunc_)
    ...
    /* Accessors */
    inline unsigned long int counts() const { return this->counter; }

    inline format::Format get_format() const { return this->fmt; }
    /* Mutators */
    inline void set_wraplen(unsigned int len) { this->wraplen = len; }

    inline void set_nowrapping() { this->set_wraplen(0); }

    inline void set_wrapping() { this->set_wraplen(DEFAULT_WRAPLEN); }

    inline void set_format(format::Format fmt_) { this->fmt = fmt_; }
    /* Methods */
    inline bool fail() const { return this->m_end == -1; }

    inline KStream& operator<<(const KSeq& rec) {
        if ((this->fmt == format::mix && rec.qual.empty()) ||  // FASTA record
            (this->fmt == format::fasta))
            this->puts('>');  // Forced FASTA
        else {
            if (rec.qual.size() != rec.seq.size()) {
                throw std::runtime_error(
                    "the sequence length doesn't match with"
    ...
    inline KStream& operator<<(format::Format fmt_) {
        this->fmt = fmt_;
        return *this;
    }

    inline KStream& operator<<(KEnd_) {
        this->flush();
        return *this;
    }
    ...
    /* Low-level methods */
    inline bool puts(std::string const& s) noexcept {
        ...
        while (cursor != s.size()) {
            ...
            len = std::min(s.size() - cursor, static_cast<std::string::size_type>(this->bufsize - this->m_begin));
            if (this->wraplen) len = std::min(len, this->wraplen - cursor % this->wraplen);
    ...
    inline bool puts(char_type c) noexcept {
        if (this->fail()) return false;
        if (this->m_begin >= this->bufsize) this->async_write();
   ...
   private:
    /* Methods */
    inline void async_write(bool term = false) noexcept {
...
class KStream<TFile, TFunc, mode::In_> : public KStreamBase_ {  // kstream_t
   ...
   protected:
    ...
    close_type close;          /**< @brief close function */
   public:
    KStream(TFile f_, TFunc func_, spec_type = mode::in, std::make_unsigned_t<size_type> bs_ = DEFAULT_BUFSIZE,
            close_type cfunc_ = nullptr)  // ks_init
        : buf(new char_type[bs_]), bufsize(bs_), f(std::move(f_)), func(std::move(func_)), close(cfunc_) {
        this->begin = 0;
        this->end = 0;
        this->is_eof = false;
        this->is_tqs = false;
    ...
    inline bool fail() const { return this->err() || this->tqs() || (this->eof() && !this->last); }

    inline KStream& operator>>(KSeq& rec)  // kseq_read
    {
        char_type c;
        this->last = false;
        ...
        while (this->getuntil(KStream::SEP_LINE, rec.qual, nullptr, true) && rec.qual.size() < rec.seq.size());
    ...
    inline std::vector<KSeq> read(std::vector<KSeq>::size_type const size) {
        std::vector<KSeq> ret;
        ret.reserve(size);
...
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, TSpec> make_kstream(TFile&& file, TFunc&& func, TSpec,
                                                                             Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, TSpec>(
        std::forward<TFile>(file), std::forward<TFunc>(func), TSpec(), std::forward<Args>(args)...);
}

template <typename TFile, typename TFunc, typename... Args>
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::In_> make_ikstream(TFile&& file, TFunc&& func,
                                                                                  Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::In_>(
        std::forward<TFile>(file), std::forward<TFunc>(func), mode::in, std::forward<Args>(args)...);
}

template <typename TFile, typename TFunc, typename... Args>
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::Out_> make_okstream(TFile&& file, TFunc&& func,
                                                                                   Args&&... args) {
    return KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::Out_>(
        std::forward<TFile>(file), std::forward<TFunc>(func), mode::out, std::forward<Args>(args)...);
}
}  // namespace klibpp
